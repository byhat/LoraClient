...
public class JniUsbSerial
{
    ...
    private static boolean getCurrentDevices()
    {
        try {
            ...
            System.out.println("[JniUsbSerial] getCurrentDevices() - Activity: " + (activity != null ? "OK" : "NULL"));
            
            if (activity == null) {
                System.out.println("[JniUsbSerial] getCurrentDevices() - Activity is NULL, returning false");
                return false;
            }

            if (usbManager == null) {
                usbManager = (UsbManager)activity.getSystemService(Context.USB_SERVICE);
                System.out.println("[JniUsbSerial] getCurrentDevices() - USB Manager initialized: " + (usbManager != null ? "OK" : "NULL"));
            }

            return true;
        } catch (Exception e) {
            System.out.println("[JniUsbSerial] getCurrentDevices() - Exception: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    public static JSONArray availableDevicesInfo()
    {
        ...
        System.out.println("[JniUsbSerial] availableDevicesInfo() called");
        
        if (!getCurrentDevices()) {
            System.out.println("[JniUsbSerial] getCurrentDevices() failed");
            return null;
        }

        System.out.println("[JniUsbSerial] USB Manager: " + (usbManager != null ? "OK" : "NULL"));
        
        if (usbManager.getDeviceList().size() < 1) {
            System.out.println("[JniUsbSerial] No USB devices found. Device list size: " + usbManager.getDeviceList().size());
            return null;
        }

        System.out.println("[JniUsbSerial] Found " + usbManager.getDeviceList().size() + " USB device(s)");
        ...
        for(UsbDevice deviceL : usbManager.getDeviceList().values()) {
            System.out.println("[JniUsbSerial] Processing device: " + deviceL.getDeviceName() +
                             " (VID: 0x" + Integer.toHexString(deviceL.getVendorId()) +
                             ", PID: 0x" + Integer.toHexString(deviceL.getProductId()) + ")");
            ...
            try {
                ...
                System.out.println("[JniUsbSerial] Exception processing device: " + e.getMessage());
                e.printStackTrace();
            }
        }

        System.out.println("[JniUsbSerial] Returning " + devicesArray.length() + " device(s)");
        return devicesArray;
    }
